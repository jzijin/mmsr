...
class SRFRDataset(data.Dataset):
    """
    ...
    def __init__(self, opt):
        super(SRFRDataset, self).__init__()
        self.opt = opt
        ...
        self.labels = {} 
        with open(opt['identity_path'], 'r') as f:
    ...
    def __getitem__(self, index):
        ...
        if self.opt['phase'] == 'train':
            ...
            rnd_h = H//2 
            rnd_w = W//2 
            img_LQ = img_LQ[rnd_h - LQ_size//2 :rnd_h + LQ_size//2, rnd_w - LQ_size//2:rnd_w + LQ_size//2, :]
            rnd_h_GT, rnd_w_GT = int(rnd_h * scale), int(rnd_w * scale)
            img_GT = img_GT[rnd_h_GT - GT_size // 2:rnd_h_GT + GT_size//2, rnd_w_GT-GT_size//2:rnd_w_GT + GT_size//2, :]
        ...
        if LQ_path is None:
            LQ_path = GT_path
        LABEL = self.labels[os.path.basename(GT_path)] 
