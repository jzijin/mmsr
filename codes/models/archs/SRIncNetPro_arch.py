...
class IncNet_block(nn.Module):
    ...
    def __init__(self, nf=64, gc=32, n_bottle_neck=3, n_res_net=5, n_rrdb=2):
        ...
        # self.upsample = UpsampleBLock(nf, 2)
...
class Basic_block(nn.Module):
    def __init__(self, nf=64, gc=32, n_bottle_neck=3, n_res_net=5, n_rrdb=2):
        ...
        basic = functools.partial(IncNet_block,nf=nf, gc=gc, n_bottle_neck=n_bottle_neck, n_res_net=n_res_net, n_rrdb=n_rrdb)
        self.basic = arch_util.make_layer(basic, 2)
        self.upsample = UpsampleBLock(nf, 2)
    def forward(self, x):
        x = self.basic(x)
        x = self.upsample(x)
...
class IncNet(nn.Module):
    ...
    """
    def __init__(self, in_nc=3, out_nc=3, nf=64, nb=5, gc=32, scale=4):
        super(IncNet, self).__init__()
        self.nums = int(math.log2(scale))
        self.conv_first = nn.Conv2d(in_nc, nf, 3, 1, 1, bias=True)

        basic_block = functools.partial(Basic_block, nf=nf, gc=gc)
        self.middle = arch_util.make_layer(basic_block, self.nums)
